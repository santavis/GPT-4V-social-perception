%% First level analyses of fMRI data with human derived regressors (Matching with the GPT4.1 data)
% Batch the first level analysis over social features
%
% Severi Santavirta & Yuhang Wu 9.6.2025

%%
% Directories
preproc_dir = '/path/preprocessed_3mm'; % Folder of the preprocessed fMRI data
regressor_dir = '/path/regressors/human_gpt41'; % Regressors
first_level_dir = '/path/first_level_human_gpt41'; % Output folder
brainmask = '/path/megafmri_localizer_gm_mask_3mm.nii'; % Brainmask
...
load('/path/regressors/human_gpt41/localizer_sub-001_human_regressors.mat');
for j = 1:size(features,2)
    feature = features{j}; % Extract the string from the cell
    
    feature_folder = sprintf('/path/tmp/%s', feature); % Save models temporarily
    
    if ~exist(feature_folder, 'dir')
        mkdir(feature_folder);
    end
    
    %% Create and temorarily save subjectwise models
    X = cell(size(subjects,1),1);
    X_nui = cell(size(subjects,1),1);
    for i = 1:size(subjects,1)
        sub = subjects{i};

        reg_file = sprintf('%s/localizer_%s_human_regressors.mat',regressor_dir,sub);
    ...
    parfor i = 1:size(subjects,1)
        ...
        con_file = sprintf('%s/beta_0001.nii',outdir);
        if(~exist(con_file,'file'))
            
            % Estimate models
            model_file = sprintf('%s/%s_model.mat',feature_folder,sub);
            scans = cellstr(spm_select('ExtFPList',preproc_dir,sprintf('%s_task-localizer_space-MNI152NLin2009cAsym_desc-',sub),1:467));
            define_first_level_model(outdir,scans,model_file,brainmask);
            estimate_first_level_model(outdir);

            % Delete unnecessary files
            delete(sprintf('%s/beta_0002.nii',outdir));
            delete(sprintf('%s/RPV.nii',outdir));
            delete(sprintf('%s/ResMS.nii',outdir));
            delete(sprintf('%s/SPM.mat',outdir));
            delete(sprintf('%s/mask.nii',outdir));

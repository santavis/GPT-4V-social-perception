%% Build GPT4.1 and human regressors for all 136 social features

% Severi Santavirta 25.5.2025

%% INPUT

vids_path = '/path/video_clips_presentation_order'; % Presented videos are named in the presentation order in this folder
datapath_gpt = '/path/data_gpt41_5batches.csv'; % GPT rating data
datapath_human = '/path/data_human_15052024.csv'; % Human rating data
datapath_log = '/path/presentation_log_files'; % fMRI presentation log files
output = '/path/regressors';
volumes_path = '/path/megafmri-localizer-num_dyns.mat'; % Volume information for each fMRI scan
...
% GPT is missing the sex scenes, delete these rows from both datasets
[itx,ida,idb] = intersect(ratings_gpt.Var1,ratings_human.Var1);
ratings_gpt = ratings_gpt(ida,:);
ratings_human = ratings_human(idb,:);
...
for sub = 1:size(subjects,1)
    ...
    % Save the data
    save(sprintf('%s/gpt41/localizer_sub-%s_gpt_regressors.mat',output,subjects{sub}),'Rgpt','TSgpt','tRgpt','tTSgpt','features');
    save(sprintf('%s/human_gpt41/localizer_sub-%s_human_regressors.mat',output,subjects{sub}),'Rhuman','TShuman','tRhuman','tTShuman','features');

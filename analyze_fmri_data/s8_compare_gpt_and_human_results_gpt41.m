%% Compare the feature specific fMRI results for social perception between GPT-4.1 and humans
...
% Define directories
gpt_dir = '/path/second_level_gpt41/'; % Input GPT results
human_dir = '/path/second_level_human_gpt41/'; % Input human results
...
for i = 1:length(common_features)
    ...
    if isfile(gpt_feature_path_fwe) && isfile(human_feature_path_fwe) && isfile(gpt_feature_path_0001) && isfile(human_feature_path_0001)
        ...
        % Load the second level unthresholded result images
...
writetable(results_table, '/path/cor_and_threshold_results_gpt41.csv');

%% Calculate cumulative maps for social perception based on GPT4.1 and human based fMRI results
...
files_gpt = find_files('/path/second_level_gpt41','spmT_0001_unc0001'); % Input GPT results
files_human = find_files('/path/second_level_human_gpt41','spmT_0001_unc0001'); % Input human results
...
V.fname = '/path/cumulative_gpt41.nii';
spm_write_vol(V,gpt_cum);
V.fname = '/path/cumulative_human_gpt41.nii';
spm_write_vol(V,human_cum);
V.fname = '/path/cumulative_difference_gpt41.nii';
spm_write_vol(V,cum_diff);

% Calculate the correlation between the cumulative maps.
img_gpt = spm_read_vols(spm_vol('/path/cumulative_gpt41.nii'));
img_human = spm_read_vols(spm_vol('/path/cumulative_human_gpt41.nii'));
img_gpt = img_gpt(:);
img_human = img_human(:);

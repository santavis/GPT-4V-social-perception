# GPT social perception: Preprocess the GPT4.1 data for video experiment
...
for(batch in seq(from=1,to=5)){
  ...
  # Load GPT data
  data_gpt <- read.csv(paste("/path/data_megaperception_clips_gpt41/batch",batch,"/batch",batch,"_data.csv",sep = ""),sep = ",")
  vids <- data_gpt$videoNames
  rownames(data_gpt) <- vids
  data_gpt <- data_gpt[-1]
...
save(data,file = "/path/data_megaperception_clips_gpt41/processed_data_5batches.RData")

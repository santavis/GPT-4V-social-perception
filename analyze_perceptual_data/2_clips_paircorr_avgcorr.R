# GPT social perception: Calculate how similarly GPT evaluated clip experiment data compared to real human participants
...
videos_human <- read.csv('path/clip_order.csv',sep = ";")
...
for(dataset in seq(from = 1,to=length(data))){
  ...
  for(set in seq(from=1,to=6)){
    ...
    clips <- intersect(clips_set,rownames(data_dataset))
    
    # Select the data for these images
    data_gpt_set <- data_dataset[clips,]
    ...
    for(feat in seq(from=1,to=ncol(data_gpt_set))){
      ...
      # Load human data
      data_human <- read.csv(paste("path/data_clips_human_individual/",features[feat],"_",set,".csv",sep=""))

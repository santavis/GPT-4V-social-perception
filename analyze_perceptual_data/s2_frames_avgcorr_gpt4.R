# GPT social perception: Calculate how similarly GPT4 evaluated frame experiment data compared to real human participants.
#
# Process:
...
for(ki in khuman){
  ...
  for(set in seq(from=1,to=6)){
    ...
    for(feat in seq(from=1,to=ncol(data_gpt_set))){
      ...
      # Load human data
      data_human <- read.csv(paste("/path/data_megaperception_frames/data_frames_human_individual/",features[feat],"_",set,".csv",sep=""))
  ...
  # Save
  write.csv(avgcorr,paste("/path/results_megaperception_frames/avgcorr_table_",ki,".csv",sep=""),row.names = T)
}

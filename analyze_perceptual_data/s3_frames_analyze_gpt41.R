# Compare the social perceptual evaluations between GPT4.1 and humans in the frame data
...
frames_human <- read.csv('/path/data_megaperception_frames/megaperception_movieframes_image_order.csv')
...
# Analyze only rows that we have in each data (some images failed in GPT data)
...
pdf("/path/results_megaperception_frames_gpt41/correlation_raw_improvement.pdf",height = 5,width = 5)
...
write.csv(iccs,"/path/results_megaperception_frames_gpt41/iccs_between_gpt_rounds.csv")
...
pdf("/path/results_megaperception_frames_gpt41/scatterplot_socialfeatures.pdf",width = 8, height = 8)
ggplot(combined_data,aes(x = human_value, y = gpt_value)) +
  ...
  labs(x = "Human average", y = "GPT-4V", title = "Images") +
...
for(ki in k){
  ...
  avg_data <- read.csv(paste("/path/results_megaperception_frames_gpt41/avgcorr_table_",ki,".csv",sep=""))
  ...
  pdf(paste("/path/results_megaperception_frames_gpt41/scatterplot_avgcorr_",ki,".pdf",sep=""),width = 20,height = 10)
  print(p)
  dev.off()
  ...
  avgcor_gpt_better[ki,3] <- mean(avg_data$gpt) # Mean cor GPT
  avgcor_gpt_better[ki,4] <- mean(avg_data$human) # Mean cor human
}
...
# Order both matrices based on the VIDEO DATASET HUMAN clustering result

# Read VIDEO CLIP human data
load('/path/data_megaperception_clips_gpt41/data_clips_human.RData')
data_nonzero_human_clips <- data

# Load frame order (data are saved in spreadsheet order and from the PicDips1 to PicDisp6)
clips_human <- read.csv('/path/data_megaperception_clips_gpt41/megaperception_clip_order.csv',sep = ";")
clips_human <- clips_human$V1
clips_human <- str_replace_all(clips_human,".mp4","")
rownames(data_nonzero_human_clips) <- clips_human

# Calculate correlation matrices
cormat_human_clips <- cor(data_nonzero_human_clips)
cormat_human <- cor(data_nonzero_human)
cormat_gpt <- cor(data_list_gpt$`1,2,3,4,5`)

# Cluster the VIDEO CLIP human correlation matrix hierarchically
cormat_human_clips_ordered <- corReorder(cormat_human_clips,order = "hclust",hclust_type = "average")

# Order the frame correlation matrices based on the previously ordered video based cormat
cormat_human_ordered <- cormat_human[colnames(cormat_human_clips_ordered),colnames(cormat_human_clips_ordered)]
cormat_gpt_ordered <- cormat_gpt[colnames(cormat_human_clips_ordered),colnames(cormat_human_clips_ordered)]

# Plot the matrices
pdf("/path/results_megaperception_frames_gpt41/gpt_cormat_ordered_by_human_clustering.pdf",width = 20,height = 20)
...
dev.off()
pdf("/path/results_megaperception_frames_gpt41/human_cormat_ordered_by_human_clustering.pdf",width = 20,height = 20)
...
# Take the first 20 components (8 significant PCs were identified for the movie data in the original experiment, but we plot more to highlight that there correlations get weak in th latter components)
...
pdf("/path/results_megaperception_frames_gpt41/PCoA_comparison_gpt_mean.pdf",width = 8,height = 8)

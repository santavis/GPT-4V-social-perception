# GPT social perception: Calculate how similarly GPT4.1 evaluated video experiment data compared to real human participants
#
# Process:
#       1. We have approximately 10 human raters -> select all possible combinations of K raters, K = {1,2,3,4,5}
#       2. Calculate the average correlation between the left-out-group and other humans
...
# Load the clip order for humans
...
for(ki in khuman){
  ...
  for(set in seq(from=1,to=6)){
    ...
    # Clips of this videoset
    clips_set <- videos_human$V1[videos_human$videoset==set]
    clips_set <- str_replace_all(clips_set,".mp4","")
    
    # Which clips we have in the GPT datasets
    clips <- intersect(clips_set,rownames(data_gpt))
    
    # Select the data for these clips
    data_gpt_set <- data_gpt[clips,]
    ...
    for(feat in seq(from=1,to=ncol(data_gpt_set))){
      ...
      # Load human data
      data_human <- read.csv(paste("/path/data_megaperception_clips_gpt41/data_clips_human_individual/",features[feat],"_",set,".csv",sep=""))
      rownames(data_human) <- clips_set
      
      # Match data with the GTP dataset
      data_human_set <- data_human[clips,]
  ...
  # Save
  write.csv(avgcorr,paste("/path/results_megaperception_clips_gpt41/avgcorr_table_",ki,".csv",sep=""),row.names = T)
}

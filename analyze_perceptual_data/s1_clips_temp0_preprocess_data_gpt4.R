# GPT social perception: Preprocess the GPT4 pilot data for video experiment when temperature is set to 0
...
data_list <- list()
for(batch in seq(from=1,to=1)){
  
  # Load GPT data
  data_gpt <- read.csv(paste("/path/data_megaperception_clips_temp0/batch",batch,"/batch",batch,"_data.csv",sep = ""),sep = ",")
  clips_gpt <- read.csv(paste("/path/data_megaperception_clips_temp0/batch",batch,"/batch",batch,"_clip_order.csv",sep = ""),header = FALSE)
  clip_codes <-  read.csv(paste("/path/data_megaperception_clips_temp0/batch",batch,"/batch",batch,"_matchtable.csv",sep = ""))
...
common_rows <- Reduce(intersect, list(rownames(data_list[[1]])))
for(batch in seq(from=1,to=1)){
...
data <- c(data_list)
save(data,file = "/path/data_megaperception_clips_temp0/processed_data_1batches.RData")

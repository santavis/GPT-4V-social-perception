"""
This is the second step in collecting image data. 
This step generates a CSV file from the json file created in the previous step.
It also checks for missing ratings for images and copies them to a new folder for an extra run of step one.
...
source_folder = 'path/stimulus/final_stimulus_set' # CHANGE
target_folder = f'path/stimulus/final_stimulus_set_round_{round_number}_{extra_round}' # CHANGE
...
# Crate a target folder
if not os.path.exists(target_folder):
    os.makedirs(target_folder)  # Create traget_folder if not exist
...
for file_name in empty_rows_content:
    ...
    if os.path.exists(source_path):
        ...
        # If the file is not found in the source folder
